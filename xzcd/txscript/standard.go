...
// Copyright (c) 2013-2017 The zcoin developers
...
// specified address.
func PayToAddrScript(addr xzcutil.Address) ([]byte, error) {
...
func MultiSigScript(pubkeys []*xzcutil.AddressPubKey, nrequired int) ([]byte, error) {
...
func ExtractPkScriptAddrs(pkScript []byte, chainParams *chaincfg.Params) (ScriptClass, []xzcutil.Address, int, error) {
	...
	case PubKeyHashTy:
		...
		requiredSigs = 1
		addr, err := xzcutil.NewAddressPubKeyHash(pops[2].data,
			chainParams)
		if err == nil {
	...
	case WitnessV0PubKeyHashTy:
		...
		// Skip the pubkey hash if it's invalid for some reason.
		requiredSigs = 1
		addr, err := xzcutil.NewAddressWitnessPubKeyHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case PubKeyTy:
		...
		requiredSigs = 1
		addr, err := xzcutil.NewAddressPubKey(pops[0].data, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case ScriptHashTy:
		...
		requiredSigs = 1
		addr, err := xzcutil.NewAddressScriptHashFromHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case WitnessV0ScriptHashTy:
		...
		// Skip the script hash if it's invalid for some reason.
		requiredSigs = 1
		addr, err := xzcutil.NewAddressWitnessScriptHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case MultiSigTy:
		...
		addrs = make([]xzcutil.Address, 0, numPubKeys)
		for i := 0; i < numPubKeys; i++ {
			addr, err := xzcutil.NewAddressPubKey(pops[i+1].data,
				chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}
...
//
// NOTE: Atomic swaps are not considered standard script types by the dcrd, zcoind
// mempool policy and should be used with P2SH.
//
...
func ExtractAtomicSwapDataPushes(pkScript []byte) (*AtomicSwapDataPushes, error) {
	...
	if pops[2].data != nil {
		secretsize, err := makeScriptNum(pops[2].data, true, 5)
		if err != nil {
			return nil, nil
		}
		pushes.SecretSize = int64(secretsize)
