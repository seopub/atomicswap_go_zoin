...
// newAddressPubKey returns a new xzcutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) xzcutil.Address {
	addr, err := xzcutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new xzcutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) xzcutil.Address {
	addr, err := xzcutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new xzcutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) xzcutil.Address {
	addr, err := xzcutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the xzcutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the xzcutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := xzcutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := xzcutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := xzcutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := xzcutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := xzcutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*xzcutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*xzcutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*xzcutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*xzcutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*xzcutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
