...
type NotificationHandlers struct {
	...
	OnRecvTx func(transaction *xzcutil.Tx, details *btcjson.BlockDetails)
	...
	//
	// NOTE: Deprecated. Use OnRelevantTxAccepted instead.
	OnRedeemingTx func(transaction *xzcutil.Tx, details *btcjson.BlockDetails)
	...
	// made to register for the notification and the function is non-nil.
	OnTxAccepted func(hash *chainhash.Hash, amount xzcutil.Amount)
	...
	// such as btcwallet.
	OnAccountBalance func(account string, balance xzcutil.Amount, confirmed bool)
...
func parseFilteredBlockConnectedParams(params []json.RawMessage) (int32,
	*wire.BlockHeader, []*xzcutil.Tx, error) {
	...
	transactions := make([]*xzcutil.Tx, len(hexTransactions))
	for i, hexTx := range hexTransactions {
		...
		transactions[i], err = xzcutil.NewTxFromBytes(transaction)
		if err != nil {
			return 0, nil, nil, err
		}
	}
...
// notifications.
func parseChainTxNtfnParams(params []json.RawMessage) (*xzcutil.Tx,
	*btcjson.BlockDetails, error) {
	...
	return xzcutil.NewTx(&msgTx), block, nil
}
...
func parseTxAcceptedNtfnParams(params []json.RawMessage) (*chainhash.Hash,
	...
	// Bounds check amount.
	amt, err := xzcutil.NewAmount(famt)
	if err != nil {
		return nil, 0, err
	}
...
func parseAccountBalanceNtfnParams(params []json.RawMessage) (account string,
	balance xzcutil.Amount, confirmed bool, err error) {
	...
	// Bounds check amount.
	bal, err := xzcutil.NewAmount(fbal)
	if err != nil {
		return "", 0, false, err
	}
...
//
// NOTE: Deprecated. Use LoadTxFilterAsync instead.
func (c *Client) NotifyReceivedAsync(addresses []xzcutil.Address) FutureNotifyReceivedResult {
	// Not supported in HTTP POST mode.
	if c.config.HTTPPostMode {
		return newFutureError(ErrWebsocketsRequired)
	}
...
//
// NOTE: Deprecated. Use LoadTxFilter instead.
func (c *Client) NotifyReceived(addresses []xzcutil.Address) error {
...
func (c *Client) RescanEndBlockAsync(startBlock *chainhash.Hash,
	addresses []xzcutil.Address, outpoints []*wire.OutPoint,
...
// NOTE: This is a btcd extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilterAsync(reload bool, addresses []xzcutil.Address,
...
// NOTE: This is a btcd extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilter(reload bool, addresses []xzcutil.Address, outPoints []wire.OutPoint) error {
