...
import (
	...
	"seopub/xzcd/integration/rpctest"
...
func makeTestOutput(r *rpctest.Harness, t *testing.T,
	amt xzcutil.Amount) (*btcec.PrivateKey, *wire.OutPoint, []byte, error) {
	...
	// spend.
	a, err := xzcutil.NewAddressPubKey(key.PubKey().SerializeCompressed(), r.ActiveNet)
	if err != nil {
		return nil, nil, nil, err
	}
...
func TestBIP0113Activation(t *testing.T) {
	...
	const outputValue = xzcutil.SatoshiPerBitcoin
	...
	txns := []*xzcutil.Tx{xzcutil.NewTx(tx)}
...
func createCSVOutput(r *rpctest.Harness, t *testing.T,
	numSatoshis xzcutil.Amount, timeLock int32,
	...
	p2shAddr, err := xzcutil.NewAddressScriptHash(csvScript, r.ActiveNet)
	if err != nil {
		return nil, nil, nil, err
	}
...
func TestBIP0068AndBIP0112Activation(t *testing.T) {
	...
	const (
		outputAmt         = xzcutil.SatoshiPerBitcoin
	...
	for txVersion := int32(0); txVersion < 3; txVersion++ {
		...
		txns := []*xzcutil.Tx{xzcutil.NewTx(spendingTx)}
		block, err := r.GenerateAndSubmitBlock(txns, -1, time.Time{})
		if err != nil {
			t.Fatalf("unable to submit block: %v", err)
		}
	...
	for i, test := range tests {
		...
		if !test.accept {
			txns := []*xzcutil.Tx{xzcutil.NewTx(test.tx)}
			_, err := r.GenerateAndSubmitBlock(txns, -1, time.Time{})
			if err == nil {
