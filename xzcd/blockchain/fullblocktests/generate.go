...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: xzcutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := xzcutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*xzcutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, xzcutil.NewTx(tx))
	}
...
func additionalCoinbase(amount xzcutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee xzcutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee xzcutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee xzcutil.Amount) *wire.MsgTx {
