...
import (
	...
	"seopub/btcsuite_websocket"
	"seopub/xzcd/blockchain"
	"seopub/xzcd/btcjson"
	"seopub/xzcd/chaincfg"
	"seopub/xzcd/chaincfg/chainhash"
	"seopub/xzcd/database"
...
// processing.
func (m *wsNotificationManager) NotifyBlockConnected(block *xzcutil.Block) {
...
func (m *wsNotificationManager) NotifyBlockDisconnected(block *xzcutil.Block) {
...
func (m *wsNotificationManager) NotifyMempoolTx(tx *xzcutil.Tx, isNew bool) {
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) addAddress(a xzcutil.Address) {
	switch a := a.(type) {
	case *xzcutil.AddressPubKeyHash:
		f.pubKeyHashes[*a.Hash160()] = struct{}{}
		return
	case *xzcutil.AddressScriptHash:
		f.scriptHashes[*a.Hash160()] = struct{}{}
		return
	case *xzcutil.AddressPubKey:
...
func (f *wsClientFilter) addAddressStr(s string, params *chaincfg.Params) {
	...
	// script.
	a, err := xzcutil.DecodeAddress(s, params)
	if err != nil {
		return
	}
	f.addAddress(a)
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) existsAddress(a xzcutil.Address) bool {
	switch a := a.(type) {
	case *xzcutil.AddressPubKeyHash:
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) removeAddress(a xzcutil.Address) {
	switch a := a.(type) {
	case *xzcutil.AddressPubKeyHash:
...
// Notification types
type notificationBlockConnected xzcutil.Block
type notificationBlockDisconnected xzcutil.Block
...
func (m *wsNotificationManager) subscribedClients(tx *xzcutil.Tx,
...
// reorganize).
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *xzcutil.Block) {
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *xzcutil.Tx) {
	...
	ntfn := btcjson.NewTxAcceptedNtfn(txHashStr, xzcutil.Amount(amount).ToBTC())
	marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
	if err != nil {
...
func blockDetails(block *xzcutil.Block, txIndex int) *btcjson.BlockDetails {
	if block == nil {
		return nil
	}
...
func newRedeemingTxNotification(txHex string, index int, block *xzcutil.Block) ([]byte, error) {
...
func (m *wsNotificationManager) notifyForTxOuts(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	addrs map[string]map[chan struct{}]*wsClient, tx *xzcutil.Tx, block *xzcutil.Block) {
...
// notifications.
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *xzcutil.Tx,
...
func (m *wsNotificationManager) notifyForTxIns(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	tx *xzcutil.Tx, block *xzcutil.Block) {
...
func checkAddressValidity(addrs []string, params *chaincfg.Params) error {
	for _, addr := range addrs {
		_, err := xzcutil.DecodeAddress(addr, params)
		if err != nil {
...
func rescanBlock(wsc *wsClient, lookups *rescanKeys, blk *xzcutil.Block) {
...
//
// NOTE: This extension is ported from github.com/decred/dcrd
func rescanBlockFilter(filter *wsClientFilter, block *xzcutil.Block, params *chaincfg.Params) []string {
...
func descendantBlock(prevHash *chainhash.Hash, curBlock *xzcutil.Block) error {
...
func handleRescan(wsc *wsClient, icmd interface{}) (interface{}, error) {
	...
	for _, addrStr := range cmd.Addresses {
		addr, err := xzcutil.DecodeAddress(addrStr, params)
		if err != nil {
			jsonErr := btcjson.RPCError{
				Code: btcjson.ErrRPCInvalidAddressOrKey,
