...
const (
	...
	MinHighPriority = xzcutil.SatoshiPerBitcoin * 144.0 / 250
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr xzcutil.Address) (*xzcutil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *xzcutil.Tx, height int32) error {
...
func logSkippedDeps(tx *xzcutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
//  |  <= policy.BlockMinSize)          |   |
//   -----------------------------------  --
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress xzcutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*xzcutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		case segwitActive && !witnessIncluded && tx.HasWitness():
			...
			// witness commitment.
			coinbaseCopy := xzcutil.NewTx(coinbaseTx.MsgTx().Copy())
	...
	block := xzcutil.NewBlock(&msgBlock)
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A xzcutil.Block should use saved in the state to avoid
