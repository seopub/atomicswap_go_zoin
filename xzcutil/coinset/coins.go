...
func (cs *CoinSet) TotalValue() (value xzcutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue xzcutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue xzcutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount xzcutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue xzcutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue xzcutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue xzcutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue xzcutil.Amount, coins []Coin) (Coins, error) {
...
// xzcutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() xzcutil.Amount {
	return xzcutil.Amount(c.txOut().Value)
}
