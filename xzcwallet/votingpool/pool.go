...
func (p *Pool) DepositScriptAddress(seriesID uint32, branch Branch, index Index) (xzcutil.Address, error) {
	script, err := p.DepositScript(seriesID, branch, index)
	if err != nil {
		return nil, err
	}
...
func (p *Pool) addressFor(script []byte) (xzcutil.Address, error) {
	scriptHash := xzcutil.Hash160(script)
	return xzcutil.NewAddressScriptHashFromHash(scriptHash, p.manager.ChainParams())
}
...
func (p *Pool) DepositScript(seriesID uint32, branch Branch, index Index) ([]byte, error) {
	...
	pks := make([]*xzcutil.AddressPubKey, len(pubKeys))
	for i, key := range pubKeys {
		...
		pks[i], err = xzcutil.NewAddressPubKey(pubkey.SerializeCompressed(),
...
func (p *Pool) addUsedAddr(seriesID uint32, branch Branch, index Index) error {
	...
	encryptedHash, err := p.manager.Encrypt(waddrmgr.CKTPublic, xzcutil.Hash160(script))
	if err != nil {
...
func (p *Pool) getUsedAddr(seriesID uint32, branch Branch, index Index) (
	...
	addr, err := xzcutil.NewAddressScriptHashFromHash(hash, mgr.ChainParams())
	if err != nil {
