...
func (s *WithdrawalStatus) Fees() xzcutil.Amount {
	return s.fees
}
...
func (o OutBailmentOutpoint) Amount() xzcutil.Amount {
	return o.amount
}
...
type withdrawalTx struct {
	...
	// struct field instead of a method so that it can be replaced in tests.
	calculateFee func() xzcutil.Amount
}
...
func newWithdrawalTx(setOptions func(tx *withdrawalTx)) *withdrawalTx {
	...
	tx.calculateFee = func() xzcutil.Amount {
		return xzcutil.Amount(1+tx.calculateSize()/1000) * feeIncrement
	}
	setOptions(tx)
	return tx
}
...
func (tx *withdrawalTx) inputTotal() (total xzcutil.Amount) {
...
func (tx *withdrawalTx) outputTotal() (total xzcutil.Amount) {
...
func (p *Pool) StartWithdrawal(roundID uint32, requests []OutputRequest,
	...
	txStore *wtxmgr.Store, chainHeight int32, dustThreshold xzcutil.Amount) (
...
func (w *withdrawal) maybeDropRequests() {
	inputAmount := xzcutil.Amount(0)
	for _, input := range w.eligibleInputs {
		inputAmount += input.Amount
	}
	outputAmount := xzcutil.Amount(0)
...
func (wi *withdrawalInfo) match(requests []OutputRequest, startAddress WithdrawalAddress,
	lastSeriesID uint32, changeStart ChangeAddress, dustThreshold xzcutil.Amount) bool {
...
func getWithdrawalStatus(p *Pool, roundID uint32, requests []OutputRequest,
	startAddress WithdrawalAddress, lastSeriesID uint32, changeStart ChangeAddress,
	dustThreshold xzcutil.Amount) (*WithdrawalStatus, error) {
...
func getRedeemScript(mgr *waddrmgr.Manager, addr *xzcutil.AddressScriptHash) ([]byte, error) {
...
func signMultiSigUTXO(mgr *waddrmgr.Manager, tx *wire.MsgTx, idx int, pkScript []byte, sigs []RawSig) error {
	...
	redeemScript, err := getRedeemScript(mgr, addresses[0].(*xzcutil.AddressScriptHash))
	if err != nil {
