...
func makeInputSource(eligible []wtxmgr.Credit) txauthor.InputSource {
	...
	currentTotal := xzcutil.Amount(0)
	currentInputs := make([]*wire.TxIn, 0, len(eligible))
	currentScripts := make([][]byte, 0, len(eligible))

	return func(target xzcutil.Amount) (xzcutil.Amount, []*wire.TxIn, [][]byte, error) {
...
func (s secretSource) GetKey(addr xzcutil.Address) (*btcec.PrivateKey, bool, error) {
...
func (s secretSource) GetScript(addr xzcutil.Address) ([]byte, error) {
	ma, err := s.Address(addr)
	if err != nil {
		return nil, err
	}
...
func (w *Wallet) txToOutputs(outputs []*wire.TxOut, account uint32, minconf int32) (*txauthor.AuthoredTx, error) {
	...
	changeSource := func() ([]byte, error) {
		...
		var changeAddr xzcutil.Address
	...
	if tx.ChangeIndex >= 0 && account == waddrmgr.ImportedAddrAccount {
		changeAmount := xzcutil.Amount(tx.Tx.TxOut[tx.ChangeIndex].Value)
