...
import (
	...
	"seopub/xzcwallet/wallet/internal/txsizes"
)
...
func makeInputSource(unspents []*wire.TxOut) InputSource {
	...
	f := func(target xzcutil.Amount) (xzcutil.Amount, []*wire.TxIn, [][]byte, error) {
		for currentTotal < target && len(unspents) != 0 {
			...
			//<-gf
			currentTotal += xzcutil.Amount(u.Value)
